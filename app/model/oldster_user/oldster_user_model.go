...
// GetIsBirthday 查询用户是否今天生日
func (o *OldsterUserModel) GetIsBirthday(userId int64) bool {
	data := IsBirthday{}
	o.Model(o).Select("SUBSTR( `card_id`,11,4) birthday, DATE_FORMAT(NOW(),'%m%d') today").Where("id = ?", userId).Scan(&data)
	if data.Birthday == data.Today {
		return true
	}
	return false
}
